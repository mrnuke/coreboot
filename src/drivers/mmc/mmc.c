 ...
 *
 * This code is severely and intentionally gilded so that it's entirely context
 * based, and there are no global lists or variables to keep track of. There is
 * no mmc_register() for this exact reason, and the way in which the mmc
 * structs are created is not specified and left to the caller. This creates a
 * bit of a non-uniform API, but at the same time allows the caller to best
 * decide how to handle the context, be it a global pointer, a struct on the
 * stack, or some other mechanism best suited to the caller's use case.
 *
 * Copyright (C) 2008,2010 Freescale Semiconductor, Inc
 *            written by Andy Fleming
 * Copyright (C) 2013,2015  Alexandru Gagniuc <mr.nuke.me@gmail.com>
...
static inline uint32_t le32_to_h(uint8_t *data)
{
	return data[0] | (data[1] << 8) | (data[2] << 16) | (data[3] << 24);
}

static inline uint32_t le24_to_h(uint8_t *data)
{
	return data[0] | (data[1] << 8) | (data[2] << 16);
}

bool mmc_getwp(struct mmc *mmc)
{
	if (mmc->board_getwp)
		return mmc->board_getwp(mmc);

	if (mmc->getwp)
		return mmc->getwp(mmc);

	return false;
}

static void iotrace_sendcmd(struct mmc_cmd *cmd)
{
	int i;
	uint8_t *ptr;
	...
	case MMC_RSP_R2:
		...
		for (i = 0; i < 4; i++) {
			...
			/* ptr is nut used if CONFIG_DEBUG_MMC is off */
			(void) ptr;
		}
		break;
	case MMC_RSP_R3:
...
static enum cb_err mmc_send_cmd(struct mmc *mmc, struct mmc_cmd *cmd,
...
static enum cb_err mmc_send_status(struct mmc *mmc, int timeout)
{
	struct mmc_cmd cmd;
	enum cb_err err;
	int retries = 5;
...
static enum cb_err mmc_set_blocklen(struct mmc *mmc, int len)
{
	struct mmc_cmd cmd;
...
static uint32_t mmc_read_blocks(struct mmc *mmc, void *dst, lbaint_t start,
				   lbaint_t blkcnt)
{
	struct mmc_cmd cmd;
	struct mmc_data data;
...
static uint32_t mmc_bread(struct block_dev_desc *dev,
			  lbaint_t start, lbaint_t blkcnt, void *dst)
{
	lbaint_t cur, blocks_todo = blkcnt;

	printk(BIOS_DEBUG, "MMC bread, no butter\n");
	if (blkcnt == 0)
		return 0;

	struct mmc *mmc = dev->priv;
	if (!mmc)
		return 0;
...
static enum cb_err mmc_app_cmd(struct mmc *mmc, uint32_t arg)
{
	struct mmc_cmd cmd;
...
static enum cb_err mmc_send_ext_csd(struct mmc *mmc, uint8_t *ext_csd)
{
	struct mmc_cmd cmd;
	struct mmc_data data;
...
static enum cb_err mmc_switch(struct mmc *mmc, uint8_t set, uint8_t index,
			      uint8_t value)
{
	struct mmc_cmd cmd;
	int timeout = 1000;
...
bool mmc_getcd(struct mmc *mmc)
{
	if (mmc->board_getcd)
		return mmc->board_getcd(mmc);

	/* Drivers are not required to provide a getcd function */
	if (mmc->getcd)
		return mmc->getcd(mmc);

	/* If there is no info to the contrary, assume a card is present */
	return true;
}

static enum cb_err sd_switch(struct mmc *mmc, int mode, int group,
...
static enum cb_err sd_change_freq(struct mmc *mmc)
{
	...
	do {
		/* FIXME: Possible endianess violation (uint32_t* -> uint8_t*) */
...
static enum cb_err mmc_startup(struct mmc *mmc)
{
	...
	uint8_t test_csd[MAX_BLOCK_LEN];
	int timeout = 1000;
...
/*
 * This is another ill-defined function, but it needs to be called in order for
 * the block_dev to receive its read() function pointer
 */
...
enum cb_err mmc_start_init(struct mmc *mmc)
{
	...
	/*
	 * FIXME: Two issues:
	 *      (1) This is more appropriate in a probe() function
	 *      (2) There could be more than one block device per controller
	 */
	mmc->block_dev.priv = mmc;
...
static enum cb_err mmc_complete_init(struct mmc *mmc)
{
	...
	mmc->init_in_progress = false;
	return err;
}

/**
 * This function is intentionally vague about how the mmc struct should be
 * created or stored. This is so that we don't have to keep a list of mmc
 * controllers, so that we don't need an mmc_register() function, and so that
 * we can avoid global variables altogether.
 */
