...
typedef struct block_dev_desc {
	...
	uint32_t(*block_read) (struct block_dev_desc *dev,
			   lbaint_t start, lbaint_t blkcnt, void *buffer);
	uint32_t(*block_write) (struct block_dev_desc *dev,
			    lbaint_t start,
			    lbaint_t blkcnt, const void *buffer);
	uint32_t(*block_erase) (struct block_dev_desc *dev,
...
struct mmc {
	...
	/* These are function pointers that board-specific code can inject into
	 * the MMC struct in order to read some board-specific status. */
	bool (*board_getwp) (struct mmc * mmc);
	bool (*board_getcd) (struct mmc * mmc);
	uint32_t b_max;
	bool op_cond_pending;	/* 1 if we are waiting on an op_cond command */
	bool init_in_progress;	/* 1 if we have done mmc_start_init() */
	uint8_t preinit;	/* start init as early as possible */
...
enum cb_err mmc_read(struct mmc *mmc, uint64_t src, uint8_t * dst, int size);
...
static inline bool mmc_host_is_spi(struct mmc *mmc)
{
	return mmc->host_caps & MMC_MODE_SPI;
}

static inline bool card_is_sd(struct mmc *mmc)
{
